...
///
/// It's simple to implement a lock free SPSC ring buffer for there won't be any
/// contention. But there are some details need to be handled to implement
/// correctly and with high efficiency. This implementation is modeled after
/// Intel DPDK::rte_ring and Linux kernel::kfifo
///
...
///   - false sharing solution learned from github.com/rigtorp/SPSCQueue and
///   github.com/MengRao/SPSC_Queue
...
class spsc_ring {
 public:
  ...
  explicit spsc_ring(const size_t capacity) noexcept : capacity_(capacity) {
    ...
    assert(reinterpret_cast<volatile char*>(&index_read_) -
               reinterpret_cast<volatile char*>(&index_write_) >=
  ...
  void emplace(Args&&... args) {
    ...
    new (static_cast<T*>(data_) + (write_index & mask_) + k_padding)
  ...
  void pop() noexcept {
    ...
    // don't really care atomicity because only one thread would do the
    // increment on r but do care about the destruction of T.
    unsigned long read_index = index_read_;
  ...
  bool batch_out(std::vector<T>& v) {
    ...
    if ((write_index & mask_) > (read_index & mask_)) {
      //  k_padding!!!!!
      ::memcpy(&v[v.size() - entries],
               static_cast<T*>(data_) + k_padding + from, entries * sizeof(T));
    }  // lost, two phase copying
    else {
      auto part1len = capacity_ - from;
      auto part2len = entries - part1len;
      ::memcpy(&v[v.size() - entries],
               static_cast<T*>(data_) + k_padding + from, part1len * sizeof(T));
      ::memcpy(&v[v.size() - part2len], static_cast<T*>(data_) + k_padding,
               part2len * sizeof(T));
    }
    //  me forgetting wrap around rules
 ...
 private:
  ...
  static constexpr size_t k_cache_line_size =
 ...
 private:
  ...
  // the point is that in the dpdk source code there indeed have volatile
  // qualifier I thought the volatile is useless in memory model before (...)
